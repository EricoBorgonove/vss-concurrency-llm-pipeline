#!/usr/bin/env python3
"""Compila e executa um benchmark C com AddressSanitizer."""
...
def build_parser():
    parser = argparse.ArgumentParser(
        description="Compila e executa um benchmark C com AddressSanitizer."
    )
    parser.add_argument("benchmark", help="Arquivo .c que sera compilado com ASAN.")
    parser.add_argument(
        "--timeout",
        type=int,
        default=10,
...
def main():
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="vss-asan-") as tmp_dir:
        ...
        print(f"Erro ao executar ASAN: {exc}", file=sys.stderr)
        print(f"Log salvo em: {display_path(log_path)}")
        return 1
