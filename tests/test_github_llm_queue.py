...
class GitHubLLMQueueTest(unittest.TestCase):
    # Verifica se a fila prioriza deteccao por ferramenta antes da revisao manual.
    def test_build_queue_rows_prioritizes_tool_detection(self):
        ...
        self.assertEqual([row["finding_id"] for row in rows], ["finding-high", "finding-confirmed"])
        self.assertEqual(rows[0]["selection_reason"], "achado detectado por ferramenta")
        self.assertEqual(rows[0]["validation_classification"], "detectado")
        self.assertEqual(rows[0]["validation_tools"], "asan")
        self.assertIn("Analise o achado de seguranca", rows[0]["prompt"])
        self.assertIn("> 8: strcpy(dst, src);", rows[0]["prompt"])
        self.assertIn("Validacoes: detectado", rows[0]["prompt"])
    ...
    def test_build_queue_from_findings_writes_csv(self):
        with TemporaryDirectory() as temp_dir:
            ...
            validations_file = root / "github_tool_validations.csv"
